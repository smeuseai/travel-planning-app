---
name: City Places Feature PRD
overview: Concise PRD + execution plan for city-based place discovery (popular/under-rated), card carousel, like/dislike click actions, saved list. Named agent chunks.
todos: []
isProject: false
---

# City Places Feature — Concise PRD + Execution Plan

## 1. Objective and Goals

- **Objective:** Discover places in chosen city (popular + under-rated), by type. Like/dislike selection. Revisit liked places.
- **Goals:** City select → places by type (Eateries, Scenic views, Paid attractions, etc.) and by popularity/under-rated → 3 images carousel + description per card → like/dislike icons, click advances to next card → saved list to refer back.
...
| Saved list       | View "Liked" places (user's own only, via RLS). Thumbnail + text. Refer back; optional remove.                                                                                                                                                               |
...
| #      | Agent name              | Scope                                                                                                                                                                 | Inputs                                                     | Outputs / contract                                                                                                                                                                                                                                                                                                     |
| ------ | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A**  | **Product Manager**     | Write `PRD.md` (or `docs/PRD.md`) with sections 1–5; lock tech choices.                                                                                               | This plan, your final answers to unresolved Qs.            | Single `PRD.md` file.                                                                                                                                                                                                                                                                                                  |
| **B**  | **Auth-Setup**          | Supabase Auth integration: sign up, sign in, sign out UI. Auth state management. Protect routes/pages.                                                                | Supabase project + Auth enabled.                           | Components: `SignUp`, `SignIn`, `SignOut`. Auth context/provider. Protected route wrapper. User session state. **Responsive** design.                                                                                                                                                                                  |
| **B2** | **City-Selector**       | City selection UI + state. No places data.                                                                                                                            | —                                                          | Selected city ID/name in app state or URL. Component: `CitySelector`; emits/updates `selectedCity`. **Responsive** design.                                                                                                                                                                                             |
| **C**  | **Places-Data**         | Google Places API: places for city + popular/under-rated. **One feed** mixing both. Map Google `types[]` to categories.                                               | `cityId`/name, optional `type`/criteria.                   | API/module: `getPlaces(city, type?)` → `Place[]`. Shape: `{ id, name, description, imageUrls[3] (good quality only), rating, user_ratings_total, category: "Hungry"                                                                                                                                                    |
| **C2** | **Backend-Likes**       | **Supabase** setup: create `likes` table (place_id, user_id, city, created_at). Use Supabase client with auth. Set up RLS policies.                                   | Supabase Auth user_id (`auth.users.id`), city.             | Supabase table + RLS: users can only access own likes **per city**. Use Supabase JS client: `supabase.from('likes').insert({place_id, user_id: user.id, city})`, `supabase.from('likes').select().eq('user_id', user.id).eq('city', city)`, `supabase.from('likes').delete().eq('id', likeId)`. RLS policies required. |
| **D**  | **Place-Card-Carousel** | One place card: 3 images carousel (good quality only) + description + **category badge** ("Hungry"/"Sight Seeing"/"Shopping") + **"Popular" or "Under-rated" badge**. | Single `Place` object (incl. `category`, `scoreCategory`). | Component: `PlaceCard`; carousel (filter low-res images); description; **category badge** ("Hungry" / "Sight Seeing" / "Shopping"); **scoreCategory badge** ("Popular" or "Under-rated"). **Responsive** design. Testable with mock `Place`.                                                                           |
| **E**  | **LikeDislike-Stack**   | Stack of cards; Like/Dislike icons; on click call backend to save like (or skip) and advance. **One feed** mixing Popular + Under-rated.                              | `Place[]` (mixed Popular + Under-rated), likes API, city.  | Component: `CardStack` or `LikeDislikeStack`; **persist likes via backend API** (include city); next card on either click. Show cards in one feed.                                                                                                                                                                     |
| **F**  | **Saved-List**          | View liked places **per city**. Fetch from backend; list (thumbnail + text); tap = detail; optional remove.                                                           | Likes API (GET list filtered by city, DELETE to remove).   | Component: `SavedList`; **reads from backend API filtered by city**; list UI; remove optional.                                                                                                                                                                                                                         |
| **G**  | **Integration-Polish**  | Wire B→B2→C→E, E↔F. Nav (auth → city → browse → saved). Empty/loading states. **Responsive** testing (desktop + mobile).                                              | All prior agents' components and contracts.                | Full flow working: Auth → City select → Browse (one feed, Popular + Under-rated badges) → Like/Dislike → Saved per city. Empty/loading copy + CTAs. **Responsive** on desktop and mobile.                                                                                                                              |


**Suggested order:** A → **B** (Auth) → B2 → C → **C2** → D → E → F → G. (D and C in parallel after B2; C2 before E/F; Auth B before all user actions.)

---

## Unresolved Questions (PRD key elements)

- **Scope:** Resolved: Saved list **per city only** (not global).
- **Data:** Resolved: Google Places API this round; others next round. Under-rated defined: `hidden_gem_score = rating / ln(1 + user_ratings_total)`. **Type mapping resolved:** Google `types[]` array → **Hungry** (food/drink + food stores), **Sight Seeing** (entertainment/culture/natural/attractions), **Shopping** (retail/malls/markets). See full mapping in Section 4.
- **Auth:** Resolved: **Full Supabase Auth** required. Users must sign up/sign in to like places. RLS policies ensure users only access their own likes.
- **Platform:** Resolved: **Responsive** (both web desktop and mobile).
- **Popular vs under-rated UI:** Resolved: **One feed only** mixing Popular + Under-rated. Show "Popular" or "Under-rated" badge on each card. No filter/tabs needed.
- **Image spec:** Resolved: Images **supplied by API** (Google Places). Only show **good quality images** (filter low-res).
- **Success metrics:** Resolved: **None for now**.
